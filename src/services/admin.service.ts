...
export class AdminService {
  ...
  async getDashboard() {
    const [
      ...
      prisma.nanny.count(),
      prisma.nanny.count({
        where: { status: NannyStatus.PENDING_VERIFICATION },
      }),
      prisma.nanny.count({ where: { status: NannyStatus.VERIFIED } }),
      prisma.booking.count(),
      prisma.booking.count({
        where: {
          status: { in: ["CONFIRMED", "NANNY_ASSIGNED", "IN_PROGRESS"] },
        },
      }),
  ...
  async getAllNannies(query: any) {
    ...
    return paginatedResult(nannies, 1, 1, 1);
  }
  ...
  async verifyNanny(nannyId: string, adminId: string, notes?: string) {
    ...
    const verifiableStatuses: NannyStatus[] = [
      NannyStatus.PENDING_VERIFICATION,
      NannyStatus.TRAINING_ASSIGNED,
    ];
    if (!verifiableStatuses.includes(nanny.status)) {
      throw new AppError(
        `Cannot verify a nanny in status: ${nanny.status}`,
        400,
      );
    }
  ...
  async rejectNanny(nannyId: string, adminId: string, reason: string) {
    ...
    if (nanny.status === NannyStatus.VERIFIED) {
      throw new AppError(
        "Cannot reject an already verified nanny. Use suspend instead.",
        400,
      );
    }
  ...
  async suspendNanny(nannyId: string, adminId: string, reason: string) {
    ...
    if (nanny.status === NannyStatus.SUSPENDED)
      throw new AppError("Nanny is already suspended", 400);
