...
const mob    = Joi.string().pattern(/^[0-9]{7,15}$/).required().messages({ 'string.pattern.base': 'mobile must be 7–15 digits' });
...
export const S = {
  ...
  otpVerify:    Joi.object({ mobile: mob, countryCode: cc, otp: Joi.string().length(6).pattern(/^\d+$/).required().messages({ 'string.length': 'OTP must be 6 digits' }) }),
  ...
  updateProfile: Joi.object({
    ...
    preferences: Joi.object({
      preferredNannyGender: Joi.string().valid('MALE', 'FEMALE', 'OTHER').allow(null),
  ...
  addAddress: Joi.object({
    ...
    coordinates:  Joi.object({ type: Joi.string(), coordinates: Joi.array().items(Joi.number()).length(2) }),
  }),
  ...
  nannyRegister: Joi.object({
    ...
    serviceTypes:     Joi.array().items(Joi.string().valid('FULL_TIME','PART_TIME','BABYSITTING','OVERNIGHT','SPECIAL_NEEDS')).min(1).required(),
    ...
    documents:        Joi.array().items(Joi.object({
      type:           Joi.string().valid('AADHAR','PAN','PASSPORT','DRIVING_LICENSE','VOTER_ID').required(),
  ...
  createBooking: Joi.object({
    nannyId:             oid(),
    serviceType:         Joi.string().valid('FULL_TIME','PART_TIME','BABYSITTING','OVERNIGHT','SPECIAL_NEEDS').required(),
    ...
    children: Joi.array().items(Joi.object({
      ...
      ageMonths:   Joi.number().integer().min(0).required(),
  ...
  cancelBooking: Joi.object({ reason: Joi.string().min(5).max(500).required() }),
  review:        Joi.object({
  ...
  sendMessage: Joi.object({
    ...
    mediaUrl: Joi.string().uri().when('type', { is: Joi.valid('IMAGE','DOCUMENT'), then: Joi.required() }),
  }),
  ...
  training:     Joi.object({ isTrainingCompleted: Joi.boolean().required(), notes: Joi.string().max(500) }),
  adminCancel:  Joi.object({ reason: Joi.string().min(5).max(500).required() }),
  refund:       Joi.object({ amount: Joi.number().min(1), reason: Joi.string().min(5).max(500).required() }),
};
