...
export async function runMidnightPlanJob() {
  ...
  log.info(
    "[midnight] Done — success: %d, failed: %d",
    results.success,
    results.failed,
  );
}
...
export async function runMorningTaskJob() {
  ...
  log.info(
    "[5am] Done — success: %d, skipped: %d, failed: %d",
    results.success,
    results.skipped,
    results.failed,
  );
...
export async function runMonthlySummaryJob() {
  ...
  log.info(
    "[monthly] Children to summarise: %d (month: %d-%d)",
    childIds.length,
    targetYear,
    targetMonth,
  );
  ...
  for (const childId of childIds) {
    try {
      await planService.generateMonthlySummary(
        childId,
        targetYear,
        targetMonth,
      );
  ...
  log.info(
    "[monthly] Done — success: %d, failed: %d",
    results.success,
    results.failed,
  );
}
...
export function registerDailyPlanJob() {
  ...
  log.info(
    "Registering midnight plan job  — schedule: %s (UTC)",
    midnightSchedule,
  );
  log.info(
    "Registering 5am task job       — schedule: %s (UTC)",
    morningSchedule,
  );
  log.info(
    "Registering monthly summary job — schedule: %s (UTC)",
    monthlySchedule,
  );
